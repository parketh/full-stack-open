...
In the example below, `notes` is defined in `index.js` and passed as a prop into `App.js`, which renders the list of notes as HTML code using the `map` function. Note that:
...
`ì` is assigned the index of the position in the array in which each note resides. However, this is **not recommend** as it can create undesirable problems.
